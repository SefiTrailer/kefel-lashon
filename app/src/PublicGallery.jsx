                        ...
                        <div className="flex flex-col gap-3 items-center text-slate-300 w-full shrink-0 mt-auto">
                            ...
                            <div className="flex flex-row justify-center items-center gap-6 xl:gap-8 bg-black/30 p-4 xl:p-6 rounded-3xl border border-white/5 shadow-inner w-full flex-wrap sm:flex-nowrap">
                                ...
                                <div className="flex flex-col items-center gap-3 w-[140px] xl:w-[150px]">
                                    <img src="./qrcode.png" alt="QR Code" className="w-[140px] xl:w-[150px] rounded-xl shadow-md bg-white border border-white/10" />
                                    <button
                                        onClick={async () => {
                    ...
                    >
                        ...
                        {(fileMetadata?.title || fileMetadata?.explanation) && (
                            <button
                                ...
                                className="absolute top-20 right-4 md:right-8 z-[80] bg-white/10 hover:bg-white/20 text-white px-4 py-2 rounded-full backdrop-blur-md transition-colors border border-white/20 shadow-md flex items-center gap-2 text-sm font-bold cursor-pointer"
                            >
                        ...
                        <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none z-[65] p-4 md:p-12">
                            <img
                                src={`./images/${encodeURIComponent(currentFile)}`}
                                alt={fileMetadata?.title || 'תמונה'}
                                className="max-w-full max-h-[90vh] xl:max-h-[95vh] object-contain drop-shadow-[0_0_60px_rgba(0,0,0,0.9)] rounded-2xl cursor-zoom-out pointer-events-auto"
                                onClick={(e) => { e.stopPropagation(); setIsFullscreen(false); }}
                            />
                        </div>

                        {/* Separate Panel for Info (Absolute Left, Floating) */}
                        {showFullscreenInfo && (fileMetadata?.title || fileMetadata?.explanation) && (
                            <div className="absolute left-4 md:left-12 top-1/2 -translate-y-1/2 w-[280px] sm:w-[320px] md:w-[380px] bg-black/80 backdrop-blur-2xl border border-white/10 rounded-3xl p-6 flex flex-col gap-5 shadow-[0_0_50px_rgba(0,0,0,0.8)] animate-in fade-in slide-in-from-left-8 z-[75] max-h-[85vh] overflow-y-auto custom-scrollbar cursor-auto">
                                {fileMetadata?.title && (
                                    <h3 className="text-2xl font-['Varela_Round',sans-serif] text-transparent bg-clip-text bg-gradient-to-r from-pink-400 to-purple-400 font-black leading-tight border-b border-white/10 pb-4 text-center">
                                        {fileMetadata.title}
                                    </h3>
                                )}
                                {fileMetadata?.explanation && (
                                    <div className="text-white/90 text-sm sm:text-base leading-relaxed flex-1 text-right dir-rtl">
                                ...
                                {fileMetadata?.explanation && (
                                    <div className="mt-4 pt-3 border-t border-white/10 text-white/40 text-xs font-medium text-right dir-rtl">
                                        * ההסבר נוסח ע"י בינה מלאכותית (AI) ועלול להכיל אי דיוקים.
                                    </div>
                                )}
                            </div>
