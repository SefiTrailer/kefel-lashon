                ...
                isFullscreen && (
                    ...
                    >
                        ...
                        <div className="w-full h-full flex items-center justify-center p-6 md:p-12 relative pointer-events-none z-50">
                            ...
                            {/* ── Fixed: Title + Toggle Button (never move) ── */}
                            <div className="hidden lg:flex absolute left-4 lg:left-6 top-8 flex-col gap-3 z-[80] pointer-events-auto w-[240px] xl:w-[280px]">

                                {/* Title pill — centered text */}
                                {fileMetadata?.title && (
                                    <div className="bg-slate-900/80 backdrop-blur-xl px-5 py-3 rounded-2xl border border-white/20 shadow-[0_0_30px_rgba(0,0,0,0.5)] text-center">
                                ...
                                {/* Toggle button — centered, green when showing, red when hiding */}
                                {fileMetadata?.explanation && (
                                    <button
                                        onClick={(e) => { e.stopPropagation(); setShowFullscreenInfo(!showFullscreenInfo); }}
                                        className={`flex items-center justify-center gap-2 px-5 py-3 rounded-2xl backdrop-blur-md transition-all text-sm font-bold shadow-[0_0_20px_rgba(0,0,0,0.3)] hover:scale-105 w-full border
                                            ${showFullscreenInfo
                                                ? 'bg-red-900/50 border-red-500/50 text-red-200 hover:bg-red-800/60'
                                                : 'bg-emerald-900/50 border-emerald-500/50 text-emerald-200 hover:bg-emerald-800/60'}`}
                                    >
                            ...
                            {/* ── Explanation panel — separate absolute, opens BELOW the button, never shifts title/button ── */}
                            {showFullscreenInfo && fileMetadata?.explanation && (
                                <div className="hidden lg:flex absolute left-4 lg:left-6 top-[8.5rem] w-[240px] xl:w-[280px] z-[79] pointer-events-auto flex-col bg-black/80 backdrop-blur-2xl border border-white/10 rounded-3xl shadow-[0_0_50px_rgba(0,0,0,0.8)] max-h-[calc(100vh-10rem)] animate-in fade-in slide-in-from-top-2 duration-200">
